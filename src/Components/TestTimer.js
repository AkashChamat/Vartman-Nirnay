...
const TestTimer = ({navigation, onTestPress}) => {
  ...
  const updateCountdownToStart = targetStartDate => {
    ...
    if (timeDiff <= 0) {
      ...
      // Trigger navigation/refresh once when countdown finishes
      try {
        if (!navigationTriggeredRef.current && nav && typeof nav.navigate === 'function') {
          navigationTriggeredRef.current = true;
          nav.navigate('ChampionSeries', {
            highlightTestId: latestPaper?.id,
            fromTimer: true,
            refreshTrigger: Date.now(),
          });
        }
      } catch (err) {
        console.warn('Failed to navigate on timer end:', err);
      }
      return;
    }
  ...
  // Handle timer press - IMPROVED: Pass testId to ChampionSeries
  const handleTimerPress = () => {
    if (!latestPaper) return;

    // If parent provided an onTestPress handler, call it first
    if (typeof onTestPress === 'function') {
      try {
        onTestPress(latestPaper);
      } catch (err) {
        // swallow handler errors but continue navigation
        console.warn('onTestPress handler threw:', err);
      }
    }

    // Use resolved navigation (prop or hook)
  ...
  // DEV: simulate timer end instantly for testing without waiting
  const renderDevSimulate = () => {
    ...
    return (
      <View style={{paddingHorizontal: 16, marginBottom: 12}}>
        <TouchableOpacity
          onPress={() => {
            try {
              if (latestPaper && nav && typeof nav.navigate === 'function') {
                console.log('DEV: Simulating timer end for test', latestPaper.id);
                navigationTriggeredRef.current = true;
                nav.navigate('ChampionSeries', {
                  highlightTestId: latestPaper.id,
                  fromTimer: true,
              ...
              console.warn('DEV simulate failed:', err);
            }
          }}
          style={{
            backgroundColor: '#333',
          ...
          <Text style={{color: '#fff', fontWeight: '700'}}>Simulate Timer End (DEV)</Text>
        </TouchableOpacity>
      </View>
    );
  };
