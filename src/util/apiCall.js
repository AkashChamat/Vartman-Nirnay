  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 404) {
        ...
        throw new Error(
          'Access denied. You do not have permission to view this user data.',
        );
      }
    }
    throw new Error(
...
export const adsExamCategory = payload =>
  getAPI(adsExamCategoryUrl, payload, null, true);
...
export const championpaper = payload =>
  getAPI(championpaperUrl, payload, null, true);
...
export const winnertitle = payload =>
  getAPI(winnertitleUrl, payload, null, true);
export const sponsortitle = payload =>
  getAPI(sponsortitleUrl, payload, null, true);
export const materialtype = payload =>
  getAPI(materialtypeUrl, payload, null, true);
export const getAllBooks = () => getAPI(getAllBooksUrl, {}, null, true);
export const getPersonalChatMobile = () => getAPI(PersonalChatUrl, {}, null, false);
export const getGroupLinks = () => getAPI(GroupUrl, {}, null, false);
export const Packages = () => getAPI(PackagesUrl, {}, null, false);


export const updateUserFcmToken = async fcmToken => {
  try {
    if (!fcmToken) {
      throw new Error('FCM token is required');
    }

    // Get user ID from storage (since backend needs it as parameter)
    const userId = await getUserId();
    if (!userId) {
      throw new Error('User ID is required for FCM token update');
    }

    // Your backend URL and endpoint
    const updateFcmTokenUrl = 'https://vartmannirnay.com:8443/updatefcmtoken';
    ...
    const fullUrl = `${updateFcmTokenUrl}?${params.toString()}`;

    const response = await postAPI(fullUrl, {}, {}, true); // Empty body, with auth
    ...
    if (error.response) {
      if (error.response.status === 401) {
        ...
        throw new Error(
          'Access denied. You do not have permission to update FCM token.',
        );
      } else if (error.response.status === 404) {
        throw new Error('User not found.');
      }
    }
    // Don't throw error here to prevent login failures
    console.warn('FCM token update failed, continuing with login...');
    return null;
  }
};
...
export const submitTestPaper = async (
  userId,
  paperId,
  answers,
  startTime,
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 401) {
        ...
        throw new Error(
          'Access denied. You do not have permission to submit this test.',
        );
      } else if (error.response.status === 404) {
...
export const getTestAnalysis = async (userId, testPaperId, type = 'ALL') => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || 'Failed to fetch question analysis',
    );
  }
};
...
const validateRegistrationPayload = payload => {
  ...
  if (!payload.contact) {
    ...
    !/^\d{10}$/.test(payload.contact.trim())
  ) {
    errors.push('contact must be exactly 10 digits');
  } else if (
    typeof payload.contact === 'number' &&
    !/^\d{10}$/.test(payload.contact.toString())
  ) {
    errors.push('contact must be exactly 10 digits');
  }
...
export const getAttemptCount = async (userId, paperId) => {
  try {
    ...
    if (error.response) {
      if (error.response.status === 404) {
        throw new Error(
          'Attempt count not found for the provided user and paper',
        );
      } else if (error.response.status === 401) {
        throw new Error('Authentication failed. Please login again.');
      } else if (error.response.status === 403) {
        throw new Error(
          'Access denied. You do not have permission to view attempt count.',
        );
      }
    }
    throw new Error(
      error.response?.data?.message || 'Failed to fetch attempt count',
    );
  }
};
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 401) {
        ...
        throw new Error(
          'Access denied. You do not have permission to view notifications.',
        );
      } else if (error.response.status === 404) {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to fetch today's notifications",
    );
  }
};
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 404) {
        ...
        throw new Error(
          'Access denied. You do not have permission to view results.',
        );
      } else if (error.response.status === 500) {
    ...
    throw new Error(
      error.response?.data?.message || 'Failed to fetch all results',
    );
  }
};
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 401) {
        ...
        throw new Error(
          'Access denied. You do not have permission to view materials.',
        );
      } else if (error.response.status === 404) {
    ...
    throw new Error(
      error.response?.data?.message || 'Failed to fetch all materials',
    );
  }
};
...
export const createTestSeriesPaymentSession = async (
  testSeriesId,
  email,
  phone,
  userId,
...
export const verifyTestSeriesPayment = async (orderId, sessionId) => {
  try {
    const response = await fetch(
      'https://your-api-url.com/verifyTestSeriesPayment',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const createBookPaymentSession = async (
  bookId,
  userId,
  email,
  phone,
    ...
    if (error.response) {
      ...
      switch (error.response.status) {
        ...
        default:
          throw new Error(
            error.response?.data?.message ||
              'Book payment order creation failed',
          );
      }
    }
  ...
  try {
    ...
    if (error.response) {
      switch (error.response.status) {
        ...
        default:
          throw new Error(
            error.response?.data?.message || 'Book payment verification failed',
          );
      }
    }
